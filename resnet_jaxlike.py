...
# Mostly copied from torchvision.models.resnet with minor modifications.
...
class Conv2DWithSamePadding(nn.Conv2d):
    """2D Convolutions like in JAX with 'SAME' padding."""
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = Conv2DWithSamePadding(
            num_channels,
            num_out_filters,
            kernel_size=7,
            stride=2,
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, Conv2DWithSamePadding):
