...
```bash
torchrun --standalone --nproc_per_node 4 main.py --arch vit_small --projector-mlp 4096-4096-256 --optimizer adamw --base-lr 4e-4 --wd 0.1 --base-target-ema 0.998 --training-fn byol_multitask --transform multitask --tasks global local cutout --num-views-per-task 2 2 1 --task-weights 1.0 1.0 1.0
```
</details>
...
```bash
torchrun --standalone --nproc_per_node 4 main.py --arch vit_base --projector-mlp 4096-4096-256 --optimizer adamw --base-lr 4e-4 --wd 0.1 --base-target-ema 0.998 --training-fn byol_multitask --transform multitask --tasks global local cutout --num-views-per-task 2 2 1 --task-weights 1.0 1.0 1.0
```
</details>